<script lang="ts">
  // Workspace shell layout — the heavy chrome (TitleBar/ActivityBar/Sidebar/
  // StatusBar), all modals, and the workspace RUNTIME listeners (shortcuts,
  // fs-events, drag-drop, auto-reopen, server settings). Document-appearance
  // concerns (CSS, theme, accent, fonts, prism) live in the root layout so the
  // dev-only /gallery — which sits OUTSIDE this (app) group — stays clean.
